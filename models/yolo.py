...
class yolo_layers(layers.Layer):
    def __init__(self,numwork=6):
        ...
        self.conv53 = layers.Conv2D(3 * (5 + self.numwork), (1, 1), padding='same', name='conv53', use_bias=False)
        self.conv54 = layers.Conv2D(3 * (5 + self.numwork), (1, 1), padding='same', name='conv54', use_bias=False)
        self.conv55 = layers.Conv2D(3 * (5 + self.numwork), (1, 1), padding='same', name='conv55', use_bias=False)
...
class yolo_Model(tf.keras.Model):
    def __init__(self):
        ...
        self.darknet_layer = yolo_layers()

    def call(self, inputs,training=None, mask=None):
        x = self.darknet_layer(inputs,training=training)
        return x
